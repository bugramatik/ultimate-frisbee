...
const int  LCD_HORIZONTAL = 16;
const int LCD_VERTICAL = 4;
...
void throwFrisbee() {
    ...
    LCDGoto(target_x, target_y); //Display target
    LCDDat(7);
    gameState.game_mode = 0; // When Frisbee flies, game is in Inactive mode
    T0CONbits.TMR0ON = 1;
...
void moveFrisbee() {
    ...
    if (current_step == 16 ||
        (frisbee_steps[current_step][0] == target_x && frisbee_steps[current_step][1] == target_y)) {
        //Frisbee reached to target
        gameState.game_mode = 1;
        current_step = 0;

        if (!isThereAnyPlayer(gameState.frisbee_position.x,
                              gameState.frisbee_position.y)) { // It should not clear player
    ...
    if (current_step != 0 &&
        !isThereAnyPlayer(gameState.frisbee_position.x, gameState.frisbee_position.y)) { // It should not clear thrower
        LCDGoto(gameState.frisbee_position.x, gameState.frisbee_position.y);
        LCDStr(" ");
    }
...
void __interrupt(high_priority)FNC()
{
    ...
    if (INTCONbits.INT0IF) { // RB0 button was pressed, throw the Frisbee
        INTCONbits.INT0IF = 0;
        if (gameState.frisbee_held) { // If Frisbee is not held by selected player don't do anything
            throwFrisbee();
        }
    ...
    if (INTCON3bits.INT1IF) { // RB1 button was pressed, change the player
        ...
        if (gameState.frisbee_held) { //If player is holding the frisbee don't switch player
            return;
        }
